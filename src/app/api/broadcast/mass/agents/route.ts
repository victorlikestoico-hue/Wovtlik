...
import { listBroadcastAgents, addBroadcastAgent } from "@/lib/db";

export async function GET(req: Request) {
	try {
		await requireRequestRole(req, authDeps, "viewer");
		return NextResponse.json(await listBroadcastAgents());
	} catch (error: any) {
...
export async function POST(req: Request) {
	try {
		...
		const agent = await addBroadcastAgent(name, phone);
